...
-- A trailing command is required for zellij to apply --cwd to new-pane,
...
keymap('n', '<leader>w<Right>', function()
  local cwd = vim.fn.getcwd()
  vim.fn.jobstart({ 'zellij', 'action', 'new-pane', '--direction', 'right', '--cwd', cwd, '--close-on-exit', '--', shell }, { detach = true })
...
keymap('n', '<leader>w<Left>', function()
  local cwd = vim.fn.getcwd()
  vim.fn.jobstart({ 'zellij', 'action', 'new-pane', '--direction', 'left', '--cwd', cwd, '--close-on-exit', '--', shell }, { detach = true })
